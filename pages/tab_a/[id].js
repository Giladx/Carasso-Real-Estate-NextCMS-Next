...
import tabAPageInitialPropsTq2bResource from '../../resources/tab_a-page-initial-props-tq_2b'
import tabAPageInitialPathsTqGYResource from '../../resources/tab_a-page-initial-paths-tq_g-y'
...
export async function getStaticProps(context) {
  try {
    const response = await tabAPageInitialPropsTq2bResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await tabAPageInitialPathsTqGYResource({
      content_type: 'tabs',
      select: 'sys.id',
    })
    return {
